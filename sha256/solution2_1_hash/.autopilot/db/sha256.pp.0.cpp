...
void sha256_final(SHA256_CTX *ctx, BYTE hash[])
{
 ...
 if (ctx->datalen < 56) {
  ctx->data[i++] = 0x80;
  sha_final_loop_1:while (i < 56)
   ctx->data[i++] = 0x00;
 }
 else {
  ctx->data[i++] = 0x80;
  sha_final_loop_2:while (i < 64)
   ctx->data[i++] = 0x00;
  compress(ctx, ctx->data);
 ...
 sha_final_loop_3:for (i = 0; i < 4; ++i) {
...
__attribute__((sdx_kernel("sha256", 0))) void sha256(SHA256_CTX *ctx, const BYTE text[]){
